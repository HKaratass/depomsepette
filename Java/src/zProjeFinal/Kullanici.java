...
public class Kullanici {
	...
	static int primEkle(String calisanSaptayici, double sepetToplamTutar) {  //PRÝM AYBAÞI SIFIRLANMALI LOGA KAYDEDÝLÝP
		int noDonusum = 0;    
		int select = 0;
		try {
...
//	static boolean basarili;
	static int ekle(String id, String ad, String soyad, String telefon, String eposta, String adres, 
			String maas, String mevki, String dgTarih) {
		int basarili = 0;
		if(id.replaceAll("\\W", "").equals("") || ad.replaceAll("\\W", "").equals("") ||
					...
					maas.replaceAll("\\W", "").equals("") || mevki.replaceAll("\\W", "").equals("") || 
			...
			if(id.replaceAll("\\d", "").replaceAll("\\W", "").equals("")) {
				...
				yeni_no+"', '"+id+"', '"+yeniSifre.kriptola(ilkSifre)+"', '"+ad+"', '"+soyad+"', '"+dgTarih+"', '"+sqlTipSaat.format(eklenenSaat)+"', '"+telefon+"', '"+eposta+"', '"+adres+"', '"+maas+"', '"+ilkPrim+"', '1');";
