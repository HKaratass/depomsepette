...
public class CalisanlarPanel implements ActionListener, MouseListener{
	...
	private void tabloCek() {
		...
		try {
			while(dbVeri.next()) {
				String adSoyad = new String(dbVeri.getString("kullanici_ad")+" "+dbVeri.getString("kullanici_soyad"));
				satir[0] = dbVeri.getInt("kullanici_no");
				satir[1] = konum;
				satir[2] = adSoyad;
				satir[3] = dbVeri.getString("kullanici_telefon");
				satir[4] = dbVeri.getString("kullanici_eposta");
				satir[5] = String.format("%,.2f", dbVeri.getDouble("kullanici_aylik_maas"))+'\u20ba';
				satir[6] = String.format("%,.2f", dbVeri.getDouble("kullanici_prim"))+'\u20ba';
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==ekleOnay) {
			...
			int basarili = Kullanici.ekle(ekleGirdiler[0].getText(), ekleGirdiler[1].getText(), ekleGirdiler[2].getText(),
											...
											ekleGirdiler[6].getText(), ekleGirdiler[7].getText(), ekleGirdiler[8].getText());
